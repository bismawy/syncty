...
import { Search, ChevronDown, Settings2, Globe } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center">
      <form onSubmit={handleSearch} className="w-full relative group">
        <div className="flex items-center w-full bg-[var(--color-card)] border border-[var(--color-border)] hover:border-[var(--color-muted-foreground)]/50 focus-within:border-[var(--color-ring)] rounded-full shadow-md transition-all p-1.5 pl-3">
          {/* Provider Selector Dropdown */}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--color-accent)]/60 hover:bg-[var(--color-accent)] text-xs font-medium text-[var(--color-foreground)] transition-colors shrink-0 select-none cursor-pointer border border-[var(--color-border)]/50"
              >
                <Globe className="h-3.5 w-3.5 text-[var(--color-muted-foreground)]" />
                <span>{activeProvider.name}</span>
                <ChevronDown className="h-3 w-3 text-[var(--color-muted-foreground)] opacity-70" />
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-48 bg-[var(--color-card)] border-[var(--color-border)]">
              {DEFAULT_PROVIDERS.map((p) => (
                <DropdownMenuItem
                  key={p.id}
                  onClick={() => setProviderId(p.id)}
                  className={`text-xs cursor-pointer flex items-center justify-between ${
                    providerId === p.id ? 'font-semibold text-[var(--color-primary)] bg-[var(--color-accent)]/50' : ''
                  }`}
                >
                  <span>{p.name}</span>
                </DropdownMenuItem>
              ...
              <DropdownMenuItem
                ...
                className={`text-xs cursor-pointer flex items-center justify-between ${
                  providerId === 'custom' ? 'font-semibold text-[var(--color-primary)] bg-[var(--color-accent)]/50' : ''
                }`}
              >
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none px-4 py-1 text-sm text-[var(--color-foreground)] placeholder-[var(--color-muted-foreground)]"
            autoFocus
          />
          ...
          <button
            type="submit"
            className="flex items-center justify-center h-9 w-9 rounded-full bg-[var(--color-accent)] hover:bg-[var(--color-accent)]/80 text-[var(--color-foreground)] border border-[var(--color-border)]/50 transition-all shrink-0 cursor-pointer shadow-xs active:scale-95"
